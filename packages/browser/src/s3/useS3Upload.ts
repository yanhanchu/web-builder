...
export function useS3Upload(): UseS3UploadResult {
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      const newItems: UploadItem[] = Array.from(files).map((file) => {
        ...
        const usesMultipart =config.multipartThresholdBytes? file.size >= (config.multipartThresholdBytes):undefined;
  ...
  const uploadOne = useCallback(
    async (item: UploadItem): Promise<UploadResult | null> => {
      ...
      try {
        const usesMultipart =config.multipartThresholdBytes? item.file.size >= config.multipartThresholdBytes:undefined;
        const uploadOptions = {
        ...
        const result = usesMultipart
          ? await uploadObjectMultipart(item.key, item.file, {
              ...uploadOptions,
              partSizeBytes: config.multipartPartSizeBytes ?? 1024*1024*5,
            })

 ...
 * `annotateNodesWithI18n`）會額外把 `PageDef.i18nBindings`（path -> key 的
 * sidecar）攤平標註回節點本身，純粹是為了讓人打開 pages.json 就能直接看出
 * 「哪個節點的哪個 prop 對應哪個 i18n key」，格式是：
 *   - 文字節點：`"foo"` -> `{ __text: "foo", __i18nKey: "home.title" }`
 *   - component 節點：`props.__i18n = { [propName]: i18nKey }`
 *
 * 這個標註不是真正的資料來源（`i18nBindings` 才是），dynamic-renderer.tsx /
 ...
 * runtime 渲染器的假設一致。
 *
 * 跟 `scripts/write-pages.mjs` 裡的 `stripI18nAnnotations` 邏輯完全對應，
 ...
 */
export function stripI18nAnnotations(nodes: PageNode[]): PageNode[] {

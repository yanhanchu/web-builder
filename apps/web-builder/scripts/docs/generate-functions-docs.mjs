 ...
 *
 * 跟 scripts/docs/generate-docs.mjs（組件文件）是同一套設計哲學，但解析器不同：
 ...
 *
 * 這支腳本位於 apps/web-builder/scripts/docs，但解析對象（src/functions/**）
 * 是 packages/ui 的原始碼；輸出則分別寫到 monorepo 根目錄的 /data，
 * 以及 apps/web-builder 自己的 src（型別定義）。
 *
 * 輸出：
 *  - /data/functions.json                                    給靜態頁面生成 / 文件 UI 用
 *  - apps/web-builder/src/types/generator/function-types.ts  FunctionDoc / ParamDoc / TypeDoc 的型別定義（供其他程式 import）
 *
 * 用法：
 *   node apps/web-builder/scripts/docs/generate-functions-docs.mjs
 */
...
// 腳本住在 apps/web-builder/scripts/docs，但要解析的函式原始碼在 packages/ui。
...
const OUTPUT_JSON = path.join(MONOREPO_ROOT, 'data', 'functions.json');
// 型別定義是給 web-builder 程式碼 import 用的，寫回 web-builder 的 src。
const OUTPUT_TYPES = path.join(WEB_BUILDER_ROOT, 'src', 'types', 'generator', 'function-types.ts');
...
function main() {
  ...
  console.log(`   → ${path.relative(MONOREPO_ROOT, OUTPUT_TYPES)}`);
...
const FUNCTION_TYPES_FILE_CONTENT = `// 此檔案由 apps/web-builder/scripts/docs/generate-functions-docs.mjs 自動產生，請勿手動編輯。
